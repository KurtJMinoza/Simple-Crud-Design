<?php
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($erru) && empty($erre) && empty($erra) && empty($errd)) {
      $sqlUpdate = "UPDATE `sample` SET username = ?, email = ?, age = ?, address = ? WHERE id = ?";
      $stmt = mysqli_prepare($conn, $sqlUpdate);
      mysqli_stmt_bind_param($stmt, "ssisi", $username, $email, $age, $address, $id);
